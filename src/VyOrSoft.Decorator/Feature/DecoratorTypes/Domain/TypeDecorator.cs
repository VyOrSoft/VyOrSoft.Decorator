...
internal class TypeDecorator : Type
{
    protected readonly Type _innerType;
    public TypeDecorator(Type innerType)
    {
        _innerType = innerType;
    }
    public override bool Equals(object? o)
        => TryResolveTypeDecoratorInnerType(o, out Type? inner) ? 
                _innerType.Equals((object)inner) : 
                _innerType.Equals(o);

    public override bool Equals(Type? o)
        => TryResolveTypeDecoratorInnerType(o, out Type? inner) ?
            _innerType.Equals(inner) :
            _innerType.Equals(o);
    public override bool IsEquivalentTo([NotNullWhen(true)] Type? other)
        => TryResolveTypeDecoratorInnerType(other, out Type? inner) ?
                _innerType.IsEquivalentTo(inner) :
                _innerType.IsEquivalentTo(other);

    public static bool operator ==(TypeDecorator? left, Type? right)
    {
    ...
    public static bool operator ==(TypeDecorator? left, TypeDecorator? right)
        => left == (Type?)right;

    public static bool operator ==(Type? left, TypeDecorator? right)
        => right == left;
    ...
    private static bool TryResolveTypeDecoratorInnerType(object? o, [NotNullWhen(true)] out Type? type)
    {
        if (o == null) return Default(out type);
        if (o is TypeDecorator typeDecorator)
        {
            type = typeDecorator._innerType;
            return true;
        }

        return Default(out type);

        static bool Default(out Type? type)
        {
            type = default;
            return false;
        }
    ...
    public override int GetHashCode() => _innerType.GetHashCode();
    public sealed override bool IsConstructedGenericType => _innerType.IsConstructedGenericType;
